...
Backend server for the XAnalyze pNodes Analytics Platform. Provides RPC proxy to bypass Cloudflare restrictions, MongoDB integration for historical data, charts API, AI assistant, and Telegram bot.
...
const response = await fetch(
  "https://your-backend.onrender.com/api/pods/devnet1"
);
...
```javascript
const response = await fetch("https://your-backend.onrender.com/api/rpc", {
  method: "POST",
...
```javascript
const response = await fetch(
  "https://your-backend.onrender.com/api/charts/network/devnet1?period=24h"
);
...
```javascript
const response = await fetch(
  "https://your-backend.onrender.com/api/charts/comparison?period=7d"
);
const { data } = await response.json();
...
```javascript
const response = await fetch("https://your-backend.onrender.com/api/pod-credits");
...
```javascript
const response = await fetch("https://your-backend.onrender.com/api/ai/status");
...
```javascript
const response = await fetch("https://your-backend.onrender.com/api/ai/ask", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
...
# Run with PM2
npm install -g pm2
pm2 start server.js --name xanalyze-backend
pm2 save
```
